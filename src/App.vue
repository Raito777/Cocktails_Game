...
-->
  <IngredientsList
    ...
    :drinksHistory="this.drinksHistory"
  ></IngredientsList>

  <!-- Pop ups -->

  <!-- Little pop up animation for a good answer (green checkmark) -->
  <div id="popUpAnimation" class="display-none">
    <svg
      width="100%"
      height="100%"
      viewBox="0 0 500 500"
    ...
    >
      <g
        transform="matrix(0.38084,-0.381137,0.480943,0.481318,-68.3166,266.544)"
      >
        <path
      ...
      <g
        transform="matrix(-0.38084,-0.381137,-0.87866,0.879346,587.31,94.0201)"
      >
        <path
  ...
  <div id="popWinAnimation" class="display-none">
    <img
      id="imgWin"
      class="display-none"
      v-bind:src="randomDrink.strDrinkThumb"
    />
...
export default {
  ...
  mounted() {
    ...
    randomCoktail.then((resultDrink) => {
      ...
      this.missingIngredients = getMissingIngredients(
        this.randomDrink,
      ...
      ingredients.then((resultIngredient) => {
        ...
        this.$refs.compositionSection.missingIngredientsWithDefaults(
          this.selectedIngredients,
  ...
  methods: {
    ...
    onOrderSubmited() {
      ...
      if (
        checkOrder(this.randomDrink, this.selectedIngredients) &&
        this.victory == false
      ) {
    ...
    updateQuizzIngredients() {
      ...
      this.quizzIngredients = get4RandomIngredientsFromType(
        this.ingredients,
      ...
      if (
        !this.quizzIngredients.some(
          (ing) => ing.idIngredient === this.missingIngredients[0].idIngredient
        )
      ) {
        const randomIndex = Math.floor(
          Math.random() * this.quizzIngredients.length
        );
    ...
    updateBarmanPhrase(good) {
      ...
      if (good) {
        if (this.selectedIngredients.length == 0) {
          this.barmanPhrase =
            startingSentencesFirstIngredient[randomIndex] +
            "<b>" +
            this.missingIngredients[0].strType.toLowerCase() +
            "</b>";
        } else if (this.missingIngredients.length == 1) {
          this.barmanPhrase =
            startingSentencesLastIngredient[randomIndex] +
            "<b>" +
            this.missingIngredients[0].strType.toLowerCase() +
            "</b>";
          ...
          this.barmanPhrase =
            startingSentencesRandomIngredient[randomIndex] +
            "<b>" +
            this.missingIngredients[0].strType.toLowerCase() +
            "</b>";
        }
    ...
    onIngredientAdded(ingredient) {
      ...
      if (checkIngredient(this.randomDrink, ingredient)) {
        ...
        const possibleDrinks = getDrinkFromIngredients(
        ...
        const index = this.quizzIngredients.findIndex(
          (ingredientObj) =>
            ingredientObj.idIngredient === ingredient.idIngredient
        );
    ...
    removeIngedient(ingredient) {
      ...
      this.isClose = false;
      this.victory = false;
      this.updateGuessedDrink(
        checkPossibleDrink(this.possibleDrinks, this.selectedIngredients)
      );
      this.onOrderSubmited();
      this.$refs.compositionSection.missingIngredientsWithDefaults(
        this.selectedIngredients,
...
const startingSentencesFirstIngredient = [
  "First, ",
  "Begin with ",
  "First we need ",
  "Start with ",
  ...
  "Firstly put some ",
  "First and foremost ",
  "In the first place put some ",
];
const startingSentencesRandomIngredient = [
  "Now put some ",
  "Keep up we need ",
  "Don't forget ",
  "Focus and add ",
...
const badIngredientSentence = [
  "You want to make sock juice ?",
  "Focus !",
  "Wrong !",
  "Please, focus",
  "It's obvious !",
...
const startingSentencesLastIngredient = [
  "Finally, all we need is ",
  "And end with ",
  "Wow, now we just need ",
  "Finish with some ",
  "Impressive, we are just missing ",
  "Therefore add ",
  "In the end we need ",
  "At last put some ",
];
...
#popUpAnimation {
  ...
  -webkit-animation: slide-in-bottom 0.3s cubic-bezier(0.25, 0.46, 0.45, 0.94)
      both,
    fade-out 0.5s ease-out both 0.6s;
